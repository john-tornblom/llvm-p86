...
class Compiler(object):
    ...
    def execute(self, args=''):
        tm = ee.TargetMachine.new(opt=self.opt_level, cm=ee.CM_JITDEFAULT)
    ...
    def save_ir(self, out, target=''):

        if self.ctx is None or self.ctx.module is None:
            return

        self.ctx.module.target = target
    ...
    def save_bit_code(self, out, target=''):
    ...
    def save_obj_code(self, out, target=''):

        if self.ctx is None or self.ctx.module is None:
            return

        tm = ee.TargetMachine.new(triple=target)
